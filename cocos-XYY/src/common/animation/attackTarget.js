...
var AttackTargetLayer=cc.Layer.extend({
	...
	drawing:function(){
		...
		this.runAction(cc.sequence(action,cc.callFunc(function(){
			if(this.targetPoint.x>=this.endPoint.x&&this.targetPoint.y>=this.endPoint.y){
...
/*var AttackTargetLayer=cc.Layer.extend({
	...
	ctor:function(usePlayer,targetPlayer,callback){
		...
		this.callBack=callback;
		this._sprite=new cc.ParticleSystem("res/drawable-hdpi/particle.plist");
		this.addChild(this._sprite);
		this._sprite.x=this.usePlayer.x;
		this._sprite.y=this.usePlayer.y;
		var action1=cc.moveTo(0.3,cc.p(this.targetPlayer.x, this.targetPlayer.y));

...
function aiSkillXuchangqing_Zhangmenren(callback) {
	...
	if (nowPlayerTerm[nowPlayerNumber].handCard > 3 || parseInt(Math.random(), 100) < 40) {
		...
		var tempHandCard = nowPlayerTerm[nowPlayerNumber].handCard[index];
		nowPlayerTerm[nowPlayerNumber].handCard.removeObject(tempHandCard);
		_friend.handCard.push(tempHandCard);
		if (_friend._name == player1._name) {
			handCardZone.pushBackCustomItem(tempHandCard);
		}
		textAreaAddMessage(nowPlayerTerm[nowPlayerNumber]._name + "发动“掌门人”效果，交给" + _friend._name + "一张牌", myText, listView, callBack);  //随机选择1张手牌交给徐长卿
	}
	if(callback!=null){
		callback();
	}
